...
D0sag3Crypt::D0sag3Crypt() : stringLength (0), vectorSize (0), digit (0), powerofDigit (0), powerofTen (0), devise (0), random7bitNum (0), random7bitbinary (0), encodedVec (0){
	// TODO Auto-generated constructor stub
}

vector<int> D0sag3Crypt::stringToVector(string a) { //Converts a string to a vector
	stringLength = a.size();
...
vector<int> D0sag3Crypt::binaryVector(vector<int> inputVector){ //Converts each term in the vector array to a binary representation of that term
...
int D0sag3Crypt::random7bit(){ //creates a random number between 0-127
	random7bitNum = rand()% 128;
	return random7bitNum;
}

int D0sag3Crypt::numberToBinary(int num) { //changes a number from base 10 format to a binary look but still in base 10 so that addition in base ten can be used on the binary number.
	random7bitbinary=0;
	for (int x = 6; x >= 0; x--) {
		powerofDigit = pow(2, x);
//		cout << "powerofDigit: " << powerofDigit << endl;
...
string D0sag3Crypt::a11b12(int x){ //converts an integer to a custom a11b12 formatted number. the first digit is in base 11 and the second in base 12.
...
string D0sag3Crypt::stringVectorToString(vector<string> a){ //converts a vector<string> (each position holding a string of length 9) to one combined string.
...
string D0sag3Crypt::cryptChars(vector<int> inputVector){ //Encrypts the characters of a vector array with a random number
...
string D0sag3Crypt::Crypt(string input){ //Encrypts a string
...
vector<string> D0sag3Crypt::cryptedStringToVector(string a) { //Converts a string after being encrypted using crypt() to a vector seperating each character into it's own position.
...
vector<string> D0sag3Crypt::seperateRandom(vector<string> input){
...
vector<int> D0sag3Crypt::vectorStringToInt(vector<string> a){
...
vector<int> D0sag3Crypt::seperateBinary(vector<string> input){
...
int D0sag3Crypt::a11b12toReg(string a11b12){
...
vector<int> D0sag3Crypt::converta11b12vecToReg(vector<string> a){
...
vector<int> D0sag3Crypt::numberVectorToBinaryVector(vector<int> a){
	vectorSize=a.size();
...
vector<int> D0sag3Crypt::DeCryptChars(vector<int> a, vector<int> b){
	vectorSize = a.size();
...
vector<int> D0sag3Crypt::binaryVectorToASCII(vector<int> a){
	vectorSize = a.size();
...
string D0sag3Crypt::intVectorToString(vector<int> a){
	vectorSize = a.size();
...
//		cout << "deCryptedVector: " << deCryptedVector << endl;
	}
	return deCryptedVector;
}

string D0sag3Crypt::DeCrypt(string input){
	string deCryptedVector;
