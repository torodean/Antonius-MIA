...
d0sEncryption::d0sEncryption() : stringLength (0), vectorSize (0), digit (0), powerofDigit (0), powerofTen (0), devise (0), random7bitNum (0), random7bitbinary (0), encodedVec (0){
}

vector<int> d0sEncryption::stringToVector(string a) { //Converts a string to a vector
    stringLength = a.size();
...
vector<int> d0sEncryption::binaryVector(vector<int> inputVector){ //Converts each term in the vector array to a binary representation of that term
...
int d0sEncryption::random7bit(){ //creates a random number between 0-127
    random7bitNum = rand()% 128;
    return random7bitNum;
}

int d0sEncryption::numberToBinary(int num) { //changes a number from base 10 format to a binary look but still in base 10 so that addition in base ten can be used on the binary number.
    random7bitbinary=0;
    for (int x = 6; x >= 0; x--) {
        powerofDigit = pow(2, x);
//		cout << "powerofDigit: " << powerofDigit << endl;
...
string d0sEncryption::a11b12(int x){ //converts an integer to a custom a11b12 formatted number. the first digit is in base 11 and the second in base 12.
...
string d0sEncryption::stringVectorToString(vector<string> a){ //converts a vector<string> (each position holding a string of length 9) to one combined string.
...
string d0sEncryption::cryptCharsNoRand(vector<int> inputVector){ //Encrypts the characters of a vector array with a random number
...
string d0sEncryption::cryptChars(vector<int> inputVector){ //Encrypts the characters of a vector array with a random number
//	cout << endl << "~~program reaches CryptChars()~~" << endl;
    bool fix=false;
    vectorSize = inputVector.size(); //sets the Int of vectorSize to the size of the input.
    vector<string> encoded (vectorSize); //Creates a vector of type strings named encoded. It is set to the vectorSize and will be used to store the encoded vectors once they are combined with the random numbers.
...
string d0sEncryption::CryptNoRand(string input){ //Encrypts a string
...
string d0sEncryption::Crypt(string input){ //Encrypts a string
    string cryptedVector;
    vector<int> baseTwoVector, inputVector;
...
vector<string> d0sEncryption::cryptedStringToVector(string a) { //Converts a string after being encrypted using crypt() to a vector seperating each character into it's own position.
...
vector<string> d0sEncryption::seperateRandom(vector<string> input){
...
vector<int> d0sEncryption::vectorStringToInt(vector<string> a){
    vectorSize = a.size();
...
vector<int> d0sEncryption::seperateBinary(vector<string> input){
...
int d0sEncryption::a11b12toReg(string a11b12){
...
vector<int> d0sEncryption::converta11b12vecToReg(vector<string> a){
...
vector<int> d0sEncryption::numberVectorToBinaryVector(vector<int> a){
    vectorSize=a.size();
...
vector<int> d0sEncryption::DeCryptChars(vector<int> a, vector<int> b){
    vectorSize = a.size();
...
vector<int> d0sEncryption::binaryVectorToASCII(vector<int> a){
    vectorSize = a.size();
...
string d0sEncryption::intVectorToString(vector<int> a){
    vectorSize = a.size();
...
string d0sEncryption::DeCrypt(string input){
    string deCryptedVector;
