...
bool Mathematics::isPrime(long number) {  //determines if a number is prime or not
...
bool Mathematics::isPalindrome(long number) {  //determines if a number is a palindrome.
...
long Mathematics::sumOfDigits(string number) { //returns the sum of the digits (as an int) within a number (entered as a string).
...
int Mathematics::numberOfFactors(long x) { //returns the number of factors in a number
    int num = 0;
    for (long i = 1; i <= x; i++) {
        if (x % i == 0) {
            num++;
...
bool Mathematics::isTriangleNumber(long x) {
    int num = 0;
    for (int i = 1; i < x; i++) {
        num += i;
        if (x == num) {
...
int Mathematics::numOfDigitsString(string x) { //returns the number of digits in a string
    return x.size();
}
...
void Mathematics::primeFactors(long x){
...
int Mathematics::numOfDigitsInt(int x){ //returns the number of digits within a non-zero integer. Negative values neglect the negative sign when given the number of digits. If the value of the input is 0 or a string of 0's the output will be zero.
    int count;
    if(x>0){
        count = log10(x)+1;
    } else if(x<0){
...
int Mathematics::setLargerInt(int One, int Two) {
    int largerInt;
...
string Mathematics::zeroString(int length) { //Creates a string of all zero's of a certain length.
    string Total;
    Total.resize(length);
...
vector<int> Mathematics::stringToVector(string a) { //Converts a string to a vector
    int length = a.size();
...
string Mathematics::vectorToString(vector<int> a) { //converts a vector of numbers to a string and removes the preceding zeros.
...
string Mathematics::resizeString(string a, string b) { //resizes a smaller string to be the same as a second string by placing zero's in front of it until it is the same length as the second string. The larger string is inputed first (a) and the smaller string to be resized is inputed as the second string (b).
...
string Mathematics::addNegativeSign(string Total){
...
string Mathematics::addTwoStrings(string stringOne, string stringTwo) { //adds 2 strings together and returns their sum as a string.
...
string Mathematics::subtractTwoStrings(string stringOne, string stringTwo) { //subtracts 2 strings together and returns their sum as a string.
...
string Mathematics::multiplyTwoStrings(string stringOne, string stringTwo) { //multiplies two strings of numbers of any length together.
...
long Mathematics::largestProductOfXConsecutiveNumbers(string string, long x) { // finds the largest product of x consecutive numbers in a string of numbers that is of length sL.
...
string Mathematics::productCollatzSequence(long n){
...
long Mathematics::latticePathsOfSquare(long x, long y){ //determines the amount of lattice paths to the bottom right corner of a square grid of size s x s
    long space[x + 1][y + 1];
    for (int i = 0; i <= x; i++) {
        for (int j = 0; j <= y; j++) {
            if (i == 0) {
