...
//        slash();
    ...
    if(!skipHold)
        SystemUtils::sleepMilliseconds(holdTime);
#endif
}
...
void VirtualKeyStrokes::two(int holdTime){
    // Press the "2" key
...
void VirtualKeyStrokes::three(int holdTime){
    // Press the "3" key
...
void VirtualKeyStrokes::four(int holdTime){
    // Press the "4" key
...
void VirtualKeyStrokes::five(int holdTime){
    // Press the "5" key
...
void VirtualKeyStrokes::six(int holdTime){
    // Press the "6" key
...
void VirtualKeyStrokes::seven(int holdTime){
    // Press the "7" key
...
void VirtualKeyStrokes::eight(int holdTime){
    // Press the "8" key
...
void VirtualKeyStrokes::nine(int holdTime){
    // Press the "9" key
...
void VirtualKeyStrokes::zero(int holdTime){
    // Press the "0" key
...
void VirtualKeyStrokes::a(int holdTime){
    // Press the "a" key
...
void VirtualKeyStrokes::leftclick(){
    INPUT Input;
    // left down
...
void VirtualKeyStrokes::rightclick(){
    INPUT Input;
    // right down
    Input.type = INPUT_MOUSE;
...
void VirtualKeyStrokes::duplicateLetter(int copies, string recipient)
{
    ...
    for(int i=0;i<copies;i++){
        type(recipient);
        SystemUtils::sleepMilliseconds(200);
        tab();
        SystemUtils::sleepMilliseconds(200);
        type("subject");
        SystemUtils::sleepMilliseconds(200);
        ...
        SystemUtils::sleepMilliseconds(200);
        paste();
        SystemUtils::sleepMilliseconds(400);
        SetCursorPos(x, y);
        SystemUtils::sleepMilliseconds(300);
        leftclick();
        SystemUtils::sleepMilliseconds(2500);
    }
}
...
void VirtualKeyStrokes::findMouseCoords(int wait){
    ...
    SystemUtils::sleepMilliseconds(wait);
    GetCursorPos(&cursor);
...
void VirtualKeyStrokes::moveMouseTo(int x, int y){
    SetCursorPos(x,y);
    SystemUtils::sleepMilliseconds(40);
}
...
void VirtualKeyStrokes::WoWFishBot(string fishButton, string lureButton)
{
    ...
    while(counter < cfg.getWoWFishBotSpace("casts")){
        ...
        if (useLure && counter % 100 == 0){
            ...
            type(lureButton);
            SystemUtils::sleepMilliseconds(3000);
        }

        //Casts.
        cout << "...Casting." << endl;
        type(fishButton);
        SystemUtils::sleepMilliseconds(1500);
        cout << "...Scanning." << endl;
        ...
        for (int j=startY;j<endY;j+=increment){
            for (int i=startX;i<endX;i+=increment){
                SetCursorPos(i,j);
                SystemUtils::sleepMilliseconds(2);
                color = GetPixel(dc, i, j);
        ...
        if(bobberFound){
            SystemUtils::sleepMilliseconds(catchDelay);
            leftclick();
        }
        SystemUtils::sleepMilliseconds(1000);
...
void VirtualKeyStrokes::buttonSpam(const std::string& button, int amount, int pause)
{
    ...
    for (int i=0;i<amount;i++){
        type(button);
        SystemUtils::sleepMilliseconds(pause);
    }
}
